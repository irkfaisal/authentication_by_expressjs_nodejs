...
export const userRegistration = async (req, res) => {
    const { email, password, name, password_conf, tc } = req.body;
    const user = await UserModal.findOne({ email: email })
    if (user) {
        res.send({ "status": "failed", message: "Email already exits" })
    } else {
        if (name && email && password && tc && password_conf) {
...
export const forgetPasswod = async (req, res) => {
    ...
    if (email) {
        ...
        if (user) {
            ...
            const link = `http://127.0.0.1:3000/api/user/forgetpassword/${user?._id}/${token}`
